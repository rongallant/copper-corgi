...
class EditGearPage extends Component {
	...
	componentDidMount() {
		...
		const gearItemDocumentRef = db
		.collection(COLLECTION_USER_GEAR).doc(userId.toString())
		.collection(COLLECTION_GEAR_ITEMS).doc(gearId.toString());

		// TODO Handle errors
		gearItemDocumentRef.get()
		.then(snapshot => {
...
export default withRouter(EditGearPage);
